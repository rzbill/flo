...
func ApplyConfig(config *Config) (Logger, error) {
	...
	// Configure handler wrappers on slog bridge
	bl = logger.(*BaseLogger)
	h, ok := bl.slogLogger.Handler().(*bridgeHandler)
	if ok {
		if len(config.RedactedFields) > 0 {
			h = h.withRedactions(config.RedactedFields)
		}
		if config.Sampling != nil && config.Sampling.Thereafter > 0 {
			h = h.withSampler(config.Sampling.Initial, config.Sampling.Thereafter)
		}
		bl.slogLogger = slog.New(h)
	}
