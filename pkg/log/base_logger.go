...
func (l *BaseLogger) Debug(msg string, fields ...Field) {
	if l.level <= DebugLevel {
		if l.slogLogger != nil {
			merged := append(attrsFromMap(l.fields), attrsFromFieldSlice(fields)...)
			l.slogLogger.LogAttrs(context.Background(), toSlogLevel(DebugLevel), msg, merged...)
			return
		}
...
func (l *BaseLogger) Info(msg string, fields ...Field) {
	if l.level <= InfoLevel {
		if l.slogLogger != nil {
			merged := append(attrsFromMap(l.fields), attrsFromFieldSlice(fields)...)
			l.slogLogger.LogAttrs(context.Background(), toSlogLevel(InfoLevel), msg, merged...)
			return
		}
...
func (l *BaseLogger) Warn(msg string, fields ...Field) {
	if l.level <= WarnLevel {
		if l.slogLogger != nil {
			merged := append(attrsFromMap(l.fields), attrsFromFieldSlice(fields)...)
			l.slogLogger.LogAttrs(context.Background(), toSlogLevel(WarnLevel), msg, merged...)
			return
		}
...
func (l *BaseLogger) Error(msg string, fields ...Field) {
	if l.level <= ErrorLevel {
		if l.slogLogger != nil {
			merged := append(attrsFromMap(l.fields), attrsFromFieldSlice(fields)...)
			l.slogLogger.LogAttrs(context.Background(), toSlogLevel(ErrorLevel), msg, merged...)
			return
		}
...
func (l *BaseLogger) Fatal(msg string, fields ...Field) {
	if l.level <= FatalLevel {
		if l.slogLogger != nil {
			merged := append(attrsFromMap(l.fields), attrsFromFieldSlice(fields)...)
			l.slogLogger.LogAttrs(context.Background(), toSlogLevel(FatalLevel), msg, merged...)
		} else {
...
func (l *BaseLogger) Debugf(msg string, args ...interface{}) {
	if l.level <= DebugLevel {
		if l.slogLogger != nil {
			merged := append(attrsFromMap(l.fields), argsToAttrs(args)...)
			l.slogLogger.LogAttrs(context.Background(), toSlogLevel(DebugLevel), msg, merged...)
			return
		}
...
func (l *BaseLogger) WithContext(ctx context.Context) Logger {
	...
	// If slog is available, attach as attrs on a derived logger
	if l.slogLogger != nil {
		child := *l
		...
		child.slogLogger = l.slogLogger.With(attrsToAny(attrsFromMap(fields))...)
		return &child
	}
...
func (l *BaseLogger) WithComponent(component string) Logger {
	if l.slogLogger != nil {
		child := *l
		child.slogLogger = l.slogLogger.With(slog.String(ComponentKey, component))
		// keep field map coherent
		child.fields = make(Fields, len(l.fields)+1)
		for k, v := range l.fields {
			child.fields[k] = v
		}
		child.fields[ComponentKey] = component
		return &child
	}
